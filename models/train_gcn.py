...
def train_gnn_bisect(args,model,etable,G,optimizer=None,scheduler=None,train=True):
    # Node features
    node_num= len(etable)
    degree = []
    for i in range(len(etable)):
    ...
    for epoch in range(1,args.epochs+1):
        ...
        local_best_ind, \
        _, \
        local_best_reward = ut.compute_bisect_reward(set_vector, etable)
        print(local_best_reward)
        print("global:",global_max_reward)

        episode_origin_mean_reward.append(np.mean(reward_vector))
        if global_max_reward < local_best_reward and len(local_best_ind) == (node_num+1)//2:  # 记录全局最好信息
            global_max_reward = local_best_reward
            global_best_ind = local_best_ind
            # global_best_ind_panel_set = local_best_panel
            best_state = model.state_dict()
...
def train_gnn_dom_k(args,model,ktable,G,optimizer=None,scheduler=None,train=True):
    # Node features
    node_num= len(ktable)
    nr_degree = []
    for i in range(len(ktable)):
        nr_degree.append(len(ktable[i]))
    nr_degree = torch.tensor(nr_degree).reshape(-1,1)
    # nr_degree = torch.softmax(nr_degree,dim=0)
    # print(nr_degree)
    # degree = G.in_degrees().reshape(-1, 1)
    ...
    for epoch in range(1,args.epochs+1):
        ...
        local_best_ind, \
        local_best_panel, \
        local_best_reward = ut.compute_dom_k_reward(set_vector, ktable)
        print("local reward:",local_best_reward)
        print("local len(panel):",len(local_best_panel))
        print("global reward:",global_max_reward)
        print("global best ind:",global_best_ind)
        ...
        l = model.dom_k_mask_loss(output, args.dom_k)
...
def train_steiner(args,model,ttable,adj,G,optimizer=None,scheduler=None,train=True):
    # Node features
    ...
    for epoch in range(1,args.epochs+1):
        ...
        # determine membership set_vector
        set_vector, set_indicator = model.decide_edge_action_mask(output, args.batch_size, adj)
        ...
        local_best_ind, \
        local_best_panel, \
        local_best_reward = ut.compute_steiner_reward(set_vector, ttable,adj)
        print("local reward:",local_best_reward)
        print("local len(panel):",len(local_best_panel))
        print("global reward:",global_max_reward)
        print("global best ind:",global_best_ind)
